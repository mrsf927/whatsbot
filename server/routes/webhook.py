        ...
        if text.startswith("["):
            try:
                ...
                if isinstance(parts, list) and all(isinstance(p, str) for p in parts):
                    ...
                    if filtered:
                        return _deduplicate_parts(filtered)
            except (json.JSONDecodeError, TypeError):
                pass
        return [reply]

    def _deduplicate_parts(parts: list[str]) -> list[str]:
        """Remove near-duplicate parts generated by the LLM (e.g. two greeting variants)."""
        if len(parts) <= 1:
            return parts
        result: list[str] = []
        for part in parts:
            ...
            for existing in result:
                ...
                # Containment: one message is fully inside the other
                if part_lower in existing_lower or existing_lower in part_lower:
                    is_dup = True
                    break
                # Character-level overlap ratio (position-aligned)
                max_len = max(len(part_lower), len(existing_lower))
                if max_len == 0:
                    continue
                matches = sum(1 for a, b in zip(part_lower, existing_lower) if a == b)
                if matches / max_len > 0.75:
                    logger.info("[Dedup] Skipping near-duplicate part: %r (similar to %r)",
                                part[:60], existing[:60])
                    is_dup = True
                    break
            if not is_dup:
